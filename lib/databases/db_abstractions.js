...
Mongoose = {
  ...
  fetchById(model, {id, projections, options = {}}, cb) {
    options.projections = projections;
    try {
      this.MODELS[model].findOne({_id: id}, options, cb);
    } catch (err) {
      cb(err);
    }
  },
  ...
  fetchOne(model, {filter, project, options = {}}, cb) {
    options.projections = project;
    try {
      this.MODELS[model].findOne(filter, options, cb);
    } catch (err) {
      cb(err);
    }
  },
  ...
  fetchMany(model, {filter, project, options = {}}, cb) {
    options.projections = project;
    try {
      this.MODELS[model].find(filter, options, cb);
    } catch (err) {
      cb(err);
    }
  },
