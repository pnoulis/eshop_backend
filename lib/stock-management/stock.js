...
INDEX = Redis.Sets,
STOCK = Redis.Hashes,
STOCK_TIMEOUT = TIME.expireIn({expireIn: "30s"});
...
export {getStock, setStock};
export
const
Stock = {
  checkExpired(stocks) {
    // example input: [1203020, 23403240, 2030]
    return stocks.map(timeOfRequest => !TIME.isExpired(timeOfRequest, STOCK_TIMEOUT) && timeOfRequest);
  },
  init() {
